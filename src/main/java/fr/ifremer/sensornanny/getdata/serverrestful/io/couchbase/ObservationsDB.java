...
public class ObservationsDB {
    ...
    public static void initialize() {
        try {
            if (!CACHE_INITIALIZED.get() && CACHE_INITIALIZING.compareAndSet(false, true)) {
                ...
                ViewQuery viewQuery = ViewQuery.from(Configuration.getInstance().observationsSyntheticViewDesign(),
                        Configuration.getInstance().observationsSyntheticGeoTemporalCountViewName()).groupLevel(3);
    ...
    public JsonObject getObservations(String bboxQuery, String timeQuery) {
        ...
        try {
            ...
            if (latDiff <= Configuration.getInstance().individualMaximumLatitudeRange() && lngDiff <= Configuration
                    .getInstance().individualMaximumLongitudeRange()) {

                SpatialViewQuery query = SpatialViewQuery.from(Configuration.getInstance()
                        .observationsIndividualViewDesign(), Configuration.getInstance()
                                .observationsIndividualGeoTemporalViewName()).startRange(JsonArray.from(
                                        queryLowerLongitude, queryLowerLatitude, queryBeginTime)).endRange(JsonArray
                                                .from(queryUpperLongitude, queryUpperLatitude, queryEndTime));

                SpatialViewResult spatialViewResult = ConnectionManager.observations.query(query, Configuration
                        .getInstance().individualQueryTimeout(), TimeUnit.MILLISECONDS);

                List<SpatialViewRow> rows = spatialViewResult.allRows(Configuration.getInstance()
                        .individualCollectTimeout(), TimeUnit.MILLISECONDS);

                if (Configuration.getInstance().individualDebug()) {
                    logger.info("SPATIAL QUERY Took " + (System.currentTimeMillis() - begin) + " ms to find " + rows
                            .size() + " matching document" + (rows.size() > 1 ? "s" : ""));
                }
                ...
                if (rows.size() <= Configuration.getInstance().individualQueryMaximumDocuments()) {
                    ...
                    if (Configuration.getInstance().individualDebug() && rows.size() == 0) {
                        logger.info("No document retrieved using bbox=" + bboxQuery + " and time=" + timeQuery
                    ...
                    if (Configuration.getInstance().individualDebug()) {
                        logger.info("SPATIAL QUERY Found " + rows.size() + "documents. Excess the limit of "
                                + Configuration.getInstance().individualQueryMaximumDocuments());
                        status = "tooMany";
                    }
                }

                if (Configuration.getInstance().individualDebug()) {
                    logger.info("SPATIAL QUERY Documents Retrieving Took " + (System.currentTimeMillis() - begin)
                            + " ms. Found " + features.size() + " measure" + (features.size() > 1 ? "s" : ""));
                }
            }
            ...
            try {
                ...
                logger.info("Too long request to retrieve obsvations using bbox=" + bboxQuery + " and time=" + timeQuery
                        + ". Took " + (System.currentTimeMillis() - begin) + " ms.");
                status = "timeOut";
    ...
    public JsonObject getObservationsCountForMapZoomUsingCache(String bboxQuery, String timeQuery) {
        ...
        for (Map.Entry<Event, Integer> each : CACHE.entrySet()) {
            ...
            if (latitude >= queryLowerLatitude && latitude <= queryUpperLatitude && longitude >= queryLowerLongitude
                    && longitude <= queryUpperLongitude && timeBegin >= queryBeginTime && timeBegin < queryEndTime) {
    ...
    public JsonArray getObservationsCountForTimelineZoomUsingCache(String bboxQuery, String timeQuery) {
